...
static GLint
_compile (char *path, GLenum type)
{
  FILE *f;
  char *text;
  long len;
...
struct Shader
shader_create (char *vs_path, char *fs_path, size_t n,
               struct VertexAttr attributes[])
{
  ...
  for (size_t i = 0; i < n; i++)
    {
      glBindAttribLocation (self.handle, attributes[i].index,
                            attributes[i].name);
    }
...
void
shader_bind (struct Shader self)
{
  glUseProgram (self.handle);
}

void
shader_uniform_mat4 (struct Shader self, char *name, mat4s m)
{
...
void
shader_uniform_view_proj (struct Shader self, struct ViewProj view_proj)
{
...
void
shader_uniform_texture2D (struct Shader self, char *name,
                          struct Texture texture, GLuint n)
{
...
void
shader_uniform_float (struct Shader self, char *name, f32 f)
{
  glUniform1f (glGetUniformLocation (self.handle, name), f);
}

void
shader_uniform_vec2 (struct Shader self, char *name, vec2s v)
{
  glUniform2f (glGetUniformLocation (self.handle, name), v.x, v.y);
}

void
shader_uniform_vec3 (struct Shader self, char *name, vec3s v)
{
  glUniform3f (glGetUniformLocation (self.handle, name), v.x, v.y, v.z);
}

void
shader_uniform_vec4 (struct Shader self, char *name, vec4s v)
{
  glUniform4f (glGetUniformLocation (self.handle, name), v.x, v.y, v.z, v.w);
}

void
shader_uniform_int (struct Shader self, char *name, int v)
{
  glUniform1i (glGetUniformLocation (self.handle, name), v);
}
void
shader_uniform_uint (struct Shader self, char *name, unsigned int v)
{
