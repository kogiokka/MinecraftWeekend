...
renderer_init (struct Renderer *self)
{
  *self = (struct Renderer){ 0 };
  ...
  self->shaders[SHADER_CHUNK] = shader_create (
      "res/shaders/chunk.vs", "res/shaders/chunk.fs", 3,
      (struct VertexAttr[]){ { .index = 0, .name = "position" },
                             { .index = 1, .name = "uv" },
                             { .index = 2, .name = "color" } });

  self->shaders[SHADER_SKY] = shader_create (
      "res/shaders/sky.vs", "res/shaders/sky.fs", 2,
      (struct VertexAttr[]){ { .index = 0, .name = "position" },
                             { .index = 1, .name = "uv" } });
  ...
  self->block_atlas
      = blockatlas_create ("res/images/blocks.png", (ivec2s){ { 16, 16 } });

  self->textures[TEXTURE_CROSSHAIR]
      = texture_create_from_path ("res/images/crosshair.png");
  self->textures[TEXTURE_CLOUDS]
      = texture_create_from_path ("res/images/clouds.png");
  self->textures[TEXTURE_STAR]
      = texture_create_from_path ("res/images/star.png");
  self->textures[TEXTURE_SUN]
      = texture_create_from_path ("res/images/sun.png");
  self->textures[TEXTURE_MOON]
      = texture_create_from_path ("res/images/moon.png");
  self->textures[TEXTURE_HOTBAR]
      = texture_create_from_path ("res/images/hotbar.png");
...
void
renderer_prepare (struct Renderer *self, enum RenderPass pass)
{
  switch (pass)
    {
    case PASS_2D:
...
void
renderer_set_camera (struct Renderer *self, enum CameraType type)
{
  self->camera_type = type;
}

void
renderer_push_camera (struct Renderer *self)
{
...
void
renderer_pop_camera (struct Renderer *self)
{
...
void
renderer_set_view_proj (struct Renderer *self)
{
...
void
renderer_use_shader (struct Renderer *self, enum ShaderType shader)
{
...
void
renderer_quad_color (struct Renderer *self, vec2s size, vec4s color,
                     mat4s model)
{
...
void
renderer_quad_texture (struct Renderer *self, struct Texture texture,
...
void
renderer_aabb (struct Renderer *self, AABB aabb, vec4s color, mat4s model,
