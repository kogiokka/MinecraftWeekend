...
static void
copy_pixels (u8 *pixels, ivec2s image_size, ivec2s size, ivec2s from,
             ivec2s to)
{
...
static void
copy_offset (struct BlockAtlas *atlas, u8 *pixels, ivec2s from, ivec2s to)
{
  copy_pixels (
      pixels, atlas->size, atlas->sprite_size,
      glms_ivec2_mul (
          atlas->sprite_size,
          (ivec2s){ { from.x, atlas->size_sprites.y - from.y - 1 } }),
      glms_ivec2_mul (atlas->sprite_size,
                      (ivec2s){ { to.x, atlas->size_sprites.y - to.y - 1 } }));
}

struct BlockAtlas
blockatlas_create (char *path, ivec2s sprite_size)
{
...
void
blockatlas_destroy (struct BlockAtlas *self)
{
  for (size_t i = 0; i < BLOCK_ATLAS_FRAMES; i++)
    {
...
void
blockatlas_update (struct BlockAtlas *self)
{
  size_t frame
      = (state.ticks / (TICKRATE / BLOCK_ATLAS_FPS)) % BLOCK_ATLAS_FRAMES;
