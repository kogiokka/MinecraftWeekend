...
void
texture_load_pixels (char *path, u8 **pixels_out, size_t *width_out,
                     size_t *height_out)
{
...
struct Texture
texture_create_from_pixels (u8 *pixels, size_t width, size_t height)
{
...
struct Texture
texture_create_from_path (char *path)
{
  u8 *pixels;
  size_t width, height;
...
void
texture_destroy (struct Texture self)
{
...
struct Atlas
atlas_create_from_texture (struct Texture texture, ivec2s sprite_size)
{
  struct Atlas self;
...
struct Atlas
atlas_create (char *path, ivec2s sprite_size)
{
  struct Atlas self;
...
void
atlas_destroy (struct Atlas self)
{
  texture_destroy (self.texture);
}

void
atlas_get (struct Atlas self, ivec2s pos, vec2s *uv_min, vec2s *uv_max)
{
  vec2s p_min = (vec2s){ { pos.x * self.sprite_size.x,
                           (self.size.y - pos.y - 1) * self.sprite_size.y } };

  *uv_min = glms_vec2_div (p_min, vec2i2f (self.texture.size));
  *uv_max = glms_vec2_div (glms_vec2_add (p_min, vec2i2f (self.sprite_size)),
                           vec2i2f (self.texture.size));
}
