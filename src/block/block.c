...
static ivec2s
get_texture_location (struct World *world, ivec3s pos, enum Direction d)
{
  assert (false);
...
static void
get_animation_frames (ivec2s out[BLOCK_ATLAS_FRAMES])
{
  return;
}

static Torchlight
get_torchlight (struct World *world, ivec3s pos)
{
  return 0x0000;
}

static void
get_aabb (struct World *world, ivec3s pos, AABB dest)
{
  dest[0] = vec3i2f (pos);
...
static void
get_mesh_information (struct World *world, ivec3s pos, enum Direction d,
