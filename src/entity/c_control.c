...
static void
update (struct ControlComponent *c_control, struct Entity entity)
{
  ...
  c_camera->camera.pitch
      = clampf32 (c_camera->camera.pitch, -PI_2 + 0.01, PI_2 - 0.01);
...
static void
tick (struct ControlComponent *c_control, struct Entity entity)
{
  ...
  c_movement->directions.forward
      = state.window->keyboard.keys[GLFW_KEY_W].down;
  c_movement->directions.backward
      = state.window->keyboard.keys[GLFW_KEY_S].down;
  ...
  c_movement->directions.down
      = state.window->keyboard.keys[GLFW_KEY_LEFT_SHIFT].down;
  ...
  struct Block held_block
      = BLOCKS[state.ui.hotbar.values[state.ui.hotbar.index]];

  if (state.window->mouse.buttons[GLFW_MOUSE_BUTTON_RIGHT].pressed_tick
      && c_blocklook->hit)
    {
      // only allow the block to be placed if it wouldn't collide with this
      // player's AABB
      ivec3s pos_place
  ...
  if (held_block.can_emit_light)
    {
      ...
      c_light->light
          = held_block.get_torchlight (entity.ecs->world, c_position->block);
    }
  else
    {
