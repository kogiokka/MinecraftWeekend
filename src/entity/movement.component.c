...
static void
tick (struct MovementComponent *c_movement, struct Entity entity)
{
  ...
  const f32 speed
      = 0.0245f * c_movement->speed * (c_movement->flags.flying ? 1.8f : 1.0f);

  vec3s movement, direction, forward, right;
  movement = GLMS_VEC3_ZERO;
  direction = GLMS_VEC3_ZERO;
  forward = (vec3s){ { sinf (c_camera->camera.yaw), 0,
                       cosf (c_camera->camera.yaw) } };
  ...
  if (c_movement->flags.flying)
    {
      if (c_movement->directions.up)
        {
          direction
              = glms_vec3_add (direction, (vec3s){ { 0.0f, 1.0f, 0.0f } });
        }

      if (c_movement->directions.down)
        {
          direction
              = glms_vec3_sub (direction, (vec3s){ { 0.0f, 1.0f, 0.0f } });
        }
    }
  else if (block.liquid)
    {
      ...
      if (c_movement->directions.up)
        {
          // increase float speed to breach surface if the top of this entity
          // is above the liquid, even more so if the entity is also colliding
          // on the x or z axes (presumably trying to exit the liquid)
          const bool breaching
              = !top_block.liquid,
              exiting
              = breaching && (c_physics->stopped.x || c_physics->stopped.z);
          const f32 float_speed = speed * 0.7f * (breaching ? 1.4f : 1.0f)
                                  * (exiting ? 2.0f : 1.0f);
  ...
  if (c_movement->flags.flying)
    {
      ...
      physics_move (c_physics, entity,
                    glms_vec3_scale (movement, flying_speed));
    }
  else
    {
