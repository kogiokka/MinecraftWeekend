...
static f32
move_axis (AABB aabb, f32 movement, vec3s axis, AABB *colliders, size_t size)
{
      ...
      if (glms_aabb_aabb (moved, colliders[i]))
        {
          ...
          f32 depth
              = glms_aabb_aabb_depth (moved, colliders[i]).raw[axis_index];
...
static vec3s
move (AABB aabb, vec3s movement, AABB *colliders, size_t size)
{
  vec3s result;
  ...
  for (size_t i = 0; i < 3; i++)
    {
      ...
      f32 movement_axis
          = move_axis (current, movement.raw[i], axis, colliders, size);
      glms_aabb_transform (
          current, glms_translate_make (glms_vec3_scale (axis, movement_axis)),
          current);
...
static void
make_aabb (struct PhysicsComponent *c_physics,
           struct PositionComponent *c_position, AABB dest)
{
  vec3s half_size = glms_vec3_divs (
      glms_vec3_sub (c_physics->size[1], c_physics->size[0]), 2.0f);
  glms_aabb_transform (
      c_physics->size,
...
static void
tick (struct PhysicsComponent *c_physics, struct Entity entity)
{
  struct PositionComponent *c_position = ecs_get (entity, C_POSITION);
  struct Block block
      = BLOCKS[world_get_block (entity.ecs->world, c_position->block)];

  if (c_physics->flags.gravity)
    {
      c_physics->velocity = glms_vec3_add (
          c_physics->velocity,
          glms_vec3_scale (GRAVITY,
                           block.solid ? 1.0f : block.gravity_modifier));
    }
  ...
  for (size_t i = 0; i < 3; i++)
    {
      bool stopped
          = fabsf (moved.raw[i] - c_physics->velocity.raw[i]) >= F32_EPSILON;
  ...
  c_physics->flags.grounded
      = c_physics->velocity.y <= 0 && c_physics->stopped.y;
...
vec3s
physics_move (struct PhysicsComponent *c_physics, struct Entity entity,
              vec3s movement)
{
  struct PositionComponent *c_position = ecs_get (entity, C_POSITION);
  ...
  c_position->position = glms_vec3_add (c_position->position, moved);
...
bool
physics_collides (struct PhysicsComponent *c_physics, AABB aabb)
{
