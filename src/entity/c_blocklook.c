...
static bool
block_raycast (struct World *world, ivec3s pos)
{
...
static void
tick (struct BlockLookComponent *c_blocklook, struct Entity entity)
{
  ...
  if (ray_block ((struct Ray){ .origin = c_camera->camera.position,
                               .direction = c_camera->camera.direction },
                 c_blocklook->radius, entity.ecs->world,
                 (bool (*) (void *, ivec3s))block_raycast, &pos, &dir))
    {
...
static void
render (struct BlockLookComponent *c_blocklook, struct Entity entity)
{
  if (c_blocklook->flags.render && c_blocklook->hit)
    {
      AABB aabb;
      BLOCKS[world_get_block (entity.ecs->world, c_blocklook->pos)].get_aabb (
          entity.ecs->world, c_blocklook->pos, aabb);
      glms_aabb_scale (aabb, (vec3s){ { 1.005f, 1.005f, 1.005f } }, aabb);
      renderer_aabb (&state.renderer, aabb,
                     (vec4s){ { 1.0f, 1.0f, 1.0f,
                                ((state.ticks % 40) > 20
                                     ? ((state.ticks % 40) / 40.0f)
                                     : (1.0f - ((state.ticks % 40) / 40.0f)))
                                    * 0.3f } },
                     glms_mat4_identity (), FILL_MODE_FILL);
    }
}
