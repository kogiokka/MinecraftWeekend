...
void
_ecs_register_internal (enum ECSComponent id, size_t component_size,
                        struct ECS *ecs, union ECSSystem system)
{
  struct ComponentList list
      = { .component_size = component_size, .system = system };
  list.components = calloc (ecs->capacity, ECSCL_ELEMENT_SIZE (&list));
  ecs->lists[id] = list;
}

void
ecs_event (struct ECS *self, enum ECSEvent event)
{
      ...
      for (size_t j = 0; j < self->capacity; j++)
        {
          ...
          if (ECS_TAG (component) & ECS_TAG_USED)
            {
              f (component, (struct Entity){
                                .id = self->ids[j], .index = j, .ecs = self });
            }
        }
    }
}
  ...
  if (i == self->capacity)
    {
      ...
      memset (((void *)self->used) + (BITMAP_SIZE_TO_BYTES (old_capacity)), 0,
              BITMAP_SIZE_TO_BYTES (self->capacity)
                  - BITMAP_SIZE_TO_BYTES (old_capacity));

      // reallocate index -> ID map, clear new allocation
      self->ids = realloc (self->ids, self->capacity * sizeof (u64));
      memset (self->ids + old_capacity, 0,
      ...
      for (size_t j = 0; j <= ECSCOMPONENT_LAST; j++)
        {
          ...
          list->components = realloc (
              list->components, self->capacity * ECSCL_ELEMENT_SIZE (list));
...
void
ecs_delete (struct ECS *self, struct Entity entity)
{
...
void
_ecs_add_internal (struct Entity entity, enum ECSComponent component_id,
                   void *value)
{
...
void
ecs_remove (struct Entity entity, enum ECSComponent component_id)
{
  struct ComponentList *list = &entity.ecs->lists[component_id];
  ECSSubscriber destroy = list->system.destroy;
  void *component = ECSCL_GET (list, entity.index);
...
bool
ecs_has (struct Entity entity, enum ECSComponent component)
{
  return ECS_TAG (ECSCL_GET (&entity.ecs->lists[component], entity.index))
         & ECS_TAG_USED;
}

void *
ecs_get (struct Entity entity, enum ECSComponent component)
{
...
void
ecs_init (struct ECS *self, struct World *world)
{
  self->capacity = 64;
