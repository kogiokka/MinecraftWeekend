...
init ()
{
  ...
  ecs_add (
      ...
      ((struct PhysicsComponent){
          .size = { (vec3s){ { 0, 0, 0 } }, (vec3s){ { 0.2f, 1.6f, 0.2f } } },
          .flags = { .gravity = true, .collide = true } }));
  ecs_add (player, C_MOVEMENT,
           ((struct MovementComponent){ .speed = 1.0f, .jump_height = 1.0f }));
  ecs_add (
      player, C_CAMERA,
      ((struct CameraComponent){ .offset = (vec3s){ { 0.0f, 0.8f, 0.0f } } }));
  ecs_add (player, C_CONTROL);
  ecs_add (player, C_BLOCKLOOK,
           ((struct BlockLookComponent){ .radius = 5.0f,
                                         .flags = { .render = true } }));
