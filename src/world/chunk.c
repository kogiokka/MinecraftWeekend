...
void
chunk_init (struct Chunk *self, struct World *world, ivec3s offset)
{
...
void
chunk_destroy (struct Chunk *self)
{
  free (self->data);
...
static void
chunk_get_bordering_chunks (struct Chunk *self, ivec3s pos,
                            struct Chunk *dest[6])
{
  size_t i = 0;
  ...
  if (pos.x == 0)
    {
      dest[i++] = world_get_chunk (
          self->world,
          glms_ivec3_add (self->offset, (ivec3s){ { -1, 0, 0 } }));
    }
  ...
  if (pos.y == 0)
    {
      dest[i++] = world_get_chunk (
          self->world,
          glms_ivec3_add (self->offset, (ivec3s){ { 0, -1, 0 } }));
    }
  ...
  if (pos.z == 0)
    {
      dest[i++] = world_get_chunk (
          self->world,
          glms_ivec3_add (self->offset, (ivec3s){ { 0, 0, -1 } }));
    }

  if (pos.x == (CHUNK_SIZE.x - 1))
    {
      dest[i++] = world_get_chunk (
          self->world, glms_ivec3_add (self->offset, (ivec3s){ { 1, 0, 0 } }));
    }

  if (pos.x == (CHUNK_SIZE.y - 1))
    {
      dest[i++] = world_get_chunk (
          self->world, glms_ivec3_add (self->offset, (ivec3s){ { 0, 1, 0 } }));
    }

  if (pos.z == (CHUNK_SIZE.z - 1))
    {
      dest[i++] = world_get_chunk (
          self->world, glms_ivec3_add (self->offset, (ivec3s){ { 0, 0, 1 } }));
    }
}

// MUST be run once a chunk has completed generating
void
chunk_after_generate (struct Chunk *self)
{
...
void
chunk_on_modify (struct Chunk *self, ivec3s pos, u64 prev, u64 data)
{
  ...
  if (data_block != prev_block)
    {
      ...
      if (block.can_emit_light)
        {
          torchlight_add (self->world, pos_w,
                          block.get_torchlight (self->world, pos_w));
        }
      ...
      if (!self->flags.generating)
        {
          if (block.transparent)
            {
              world_heightmap_recalculate (self->world,
                                           (ivec2s){ { pos_w.x, pos_w.z } });
...
void
chunk_prepare (struct Chunk *self)
{
  if (self->flags.empty)
    {
      return;
...
void
chunk_render (struct Chunk *self, enum ChunkMeshPart part)
{
  if (self->flags.empty)
    {
      return;
...
void
chunk_update (struct Chunk *self)
{
  ...
  struct PositionComponent *c_position
      = ecs_get (self->world->entity_view, C_POSITION);
  bool within_distance
      = glms_ivec3_norm (glms_ivec3_sub (self->offset, c_position->offset))
        < 4;

  self->mesh->flags.depth_sort
      = (!ivec3scmp (self->offset, c_position->offset)
         && c_position->block_changed)
        || (c_position->offset_changed && within_distance);

  // Persist depth sort data if the player is within depth sort distance of
  // this chunk
  chunkmesh_set_persist (self->mesh, within_distance);
}

void
chunk_tick (struct Chunk *self)
{
}