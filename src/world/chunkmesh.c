...
// gets data at the specified position in the chunk if the position is inside
// of the chunk, otherwise gets data from the world
...
static void
buffer_init (struct ChunkMeshBuffer *self, enum BufferType type)
{
...
static void
buffer_destroy (struct ChunkMeshBuffer *self)
{
  if (self->data != NULL)
    {
      free (self->data);
...
struct ChunkMesh *
chunkmesh_create (struct Chunk *chunk)
{
...
void
chunkmesh_destroy (struct ChunkMesh *self)
{
...
static void
buffer_prepare (struct ChunkMeshBuffer *self)
{
  if (self->data == NULL)
    {
...
void
chunkmesh_prepare (struct ChunkMesh *self)
{
  self->vertex_count = 0;
...
static void
chunkmesh_flip_buffers (struct ChunkMesh *self)
{
...
static void
chunkmesh_finalize_data (struct ChunkMesh *self)
{
  assert (self->buffers[DATA].data != NULL);
  vbo_buffer (self->vbo, self->buffers[DATA].data, 0,
...
static void
chunkmesh_finalize_indices (struct ChunkMesh *self)
{
...
// previously and only reorders indices in the transparent portion of the
// buffer
enum SortKind
{
  SORT_FULL,
...
static int
depth_cmp (const struct Face *a, const struct Face *b)
{
  return (int)-sign (a->distance - b->distance);
}

static void
chunkmesh_sort (struct ChunkMesh *self, enum SortKind kind)
{
  ...
  for (size_t i = 0; i < faces->elements; i++)
    {
      ...
      face->distance
          = glms_vec3_norm2 (glms_vec3_sub (center, face->position));
    }
  ...
  // iterate over sorted faces: move them (in order) into the front of the
  // index buffer
  for (size_t i = 0; i < faces->elements; i++)
    {
      struct Face *face = &((struct Face *)faces->data)[i];
  ...
  if (kind == SORT_FULL)
    {
      ...
      // iterate over ALL faces, move those that weren't originally moved to
...
static void
chunkmesh_mesh (struct ChunkMesh *self)
{
                ...
                if (params.block->mesh_type == BLOCKMESH_CUSTOM
                        ...
                        && params.block_n->id != params.block->id))
                  {
                    ...
                    atlas_get (block_atlas->atlas,
                               params.block->get_texture_location (
                                   chunk->world, pos_w, d),
                               &params.uv_min, &params.uv_max);
                    ...
                    if (params.block->mesh_type == BLOCKMESH_CUSTOM)
                      {
                        ...
                        params.block->get_mesh_information (
                            ...
                            &params.offset, &params.size, &uv_offset_px,
                            &uv_size_px);

                        // offset UV coordinates
                        vec2s uv_offset
                            = glms_vec2_mul (vec2i2f (uv_offset_px),
                                             block_atlas->atlas.pixel_unit),
                            uv_size
                            = glms_vec2_mul (vec2i2f (uv_size_px),
                                             block_atlas->atlas.pixel_unit);

                        params.uv_min
                            = glms_vec2_add (params.uv_min, uv_offset);
...
void
chunkmesh_prepare_render (struct ChunkMesh *self)
{
...
void
chunkmesh_render (struct ChunkMesh *self, enum ChunkMeshPart part)
{
  ...
  vao_attr (self->vao, self->vbo, 0, 3, GL_FLOAT, vertex_size,
            0 * sizeof (f32));
  vao_attr (self->vao, self->vbo, 1, 2, GL_FLOAT, vertex_size,
            3 * sizeof (f32));
...
void
chunkmesh_set_persist (struct ChunkMesh *self, bool persist)
{
