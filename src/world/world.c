...
#define NUM_CHUNKS(_w)                                                        \
  ((_w)->chunks_size * (_w)->chunks_size * (_w)->chunks_size)
...
static int
_btf_pcmp (const ivec3s **a, const ivec3s **b, ivec3s *center)
{
  return -(glms_ivec3_norm2 (glms_ivec3_sub (*center, **a))
           - glms_ivec3_norm2 (glms_ivec3_sub (*center, **b)));
}

// front-to-back ordering pointer comparator
static int
_ftb_pcmp (const ivec3s **a, const ivec3s **b, ivec3s *center)
{
  return glms_ivec3_norm2 (glms_ivec3_sub (*center, **a))
         - glms_ivec3_norm2 (glms_ivec3_sub (*center, **b));
}
...
static int
_btf_cmp (const ivec3s *a, const ivec3s *b, ivec3s *center)
{
  return -(glms_ivec3_norm2 (glms_ivec3_sub (*center, *a))
           - glms_ivec3_norm2 (glms_ivec3_sub (*center, *b)));
}
...
static int
_ftb_cmp (const ivec3s *a, const ivec3s *b, ivec3s *center)
{
  return -(glms_ivec3_norm2 (glms_ivec3_sub (*center, *b))
           - glms_ivec3_norm2 (glms_ivec3_sub (*center, *a)));
}
...
#define _world_foreach_cmp_impl(_w, _cname, _cmp, _v0, _v1, _v2)              \
  ...
  sort_r (_v2, _v0, sizeof (ivec3s *),                                        \
          (int (*) (const void *, const void *, void *))_cmp,                 \
          &_w->center_offset);                                                \
  struct Chunk *_cname;                                                       \
  for (size_t i = 0;                                                          \
       i < _v0                                                                \
       && (_cname = (_w)->chunks[world_chunk_index (_w, *_v2[i])])            \
              != (void *)INT64_MAX;                                           \
       i++)

#define _world_foreach_offset_cmp_impl(_w, _iname, _oname, _cmp, _v0, _v1,    \
                                       _v2)                                   \
...
#define world_foreach_btf(_w, _cname)                                         \
  _world_foreach_cmp_impl (_w, _cname, _btf_pcmp, CONCAT (v, __COUNTER__),    \
                           CONCAT (v, __COUNTER__), CONCAT (v, __COUNTER__))

// iterate chunks from the center out (front to back)
#define world_foreach_ftb(_w, _cname)                                         \
  _world_foreach_cmp_impl (_w, _cname, _ftb_pcmp, CONCAT (v, __COUNTER__),    \
                           CONCAT (v, __COUNTER__), CONCAT (v, __COUNTER__))

// iterate OFFSETS from the borders in (back to front)
#define world_foreach_offset_btf(_w, _iname, _oname)                          \
  _world_foreach_offset_cmp_impl (                                            \
      _w, _iname, _oname, _btf_cmp, CONCAT (v, __COUNTER__),                  \
      CONCAT (v, __COUNTER__), CONCAT (v, __COUNTER__))

// iterate OFFSETS from the center out (front to back)
#define world_foreach_offset_ftb(_w, _iname, _oname)                          \
  _world_foreach_offset_cmp_impl (                                            \
      _w, _iname, _oname, _ftb_cmp, CONCAT (v, __COUNTER__),                  \
      CONCAT (v, __COUNTER__), CONCAT (v, __COUNTER__))

// world block position -> heightmap internal position
static inline ivec2s
world_pos_to_heightmap_pos (ivec2s pos)
{
  return glms_ivec2_mod (
      glms_ivec2_add (glms_ivec2_mod (pos, CHUNK_SIZE_XZ), CHUNK_SIZE_XZ),
      CHUNK_SIZE_XZ);
}

static inline bool
world_heightmap_in_bounds (struct World *self, ivec2s offset)
{
  ivec2s p = glms_ivec2_sub (
      offset, (ivec2s){ { self->chunks_origin.x, self->chunks_origin.z } });
  return p.x >= 0 && p.y >= 0 && p.x < (i32)self->chunks_size
         && p.y < (i32)self->chunks_size;
}

// heightmap offset -> array index
static inline size_t
world_heightmap_index (struct World *self, ivec2s offset)
{
  ivec2s p = glms_ivec2_sub (
      offset, (ivec2s){ { self->chunks_origin.x, self->chunks_origin.z } });
...
static inline ivec2s
world_heightmap_offset (struct World *self, size_t i)
{
  return (ivec2s){ { self->chunks_origin.x + (i / self->chunks_size),
                     self->chunks_origin.z + (i % self->chunks_size) } };
}

// returns the heightmap for the specified chunk
struct Heightmap *
chunk_get_heightmap (struct Chunk *self)
{
  return self->world->heightmaps[world_heightmap_index (
      self->world, (ivec2s){ { self->offset.x, self->offset.z } })];
}

// returns heightmap value at specified x, z coordinate
i64
world_heightmap_get (struct World *self, ivec2s p)
{
...
void
world_heightmap_set (struct World *self, ivec2s p, i64 y)
{
  ivec3s offset = world_pos_to_offset ((ivec3s){ { p.x, 0, p.y } });
  HEIGHTMAP_SET (self->heightmaps[world_heightmap_index (
                     self, (ivec2s){ { offset.x, offset.z } })],
...
bool
world_heightmap_update (struct World *self, ivec3s p)
{
  ivec3s offset = world_pos_to_offset (p);
  struct Heightmap *h = self->heightmaps[world_heightmap_index (
      self, (ivec2s){ { offset.x, offset.z } })];
  size_t i = HEIGHTMAP_INDEX (
      world_pos_to_heightmap_pos ((ivec2s){ { p.x, p.z } }));
...
void
chunk_heightmap_recalculate (struct Chunk *chunk)
{
      ...
      for (i64 z = 0; z < CHUNK_SIZE.z; z++)
        {
          ...
          // don't set heightmap value if it's already above this
          // column
          ...
          for (i64 y = CHUNK_SIZE.y - 1; y >= 0; y--)
            {
              ...
              if (!BLOCKS[chunk_get_block (chunk, pos_c)].transparent)
                {
                  HEIGHTMAP_SET (heightmap, ((ivec2s){ { pos_c.x, pos_c.z } }),
                                 pos_w.y);
                  break;
                }
            }
...
void
world_heightmap_recalculate (struct World *self, ivec2s p)
{
...
void
world_load_chunk (struct World *self, ivec3s offset)
{
  ...
  for (size_t i = 0; i < self->unloaded_blocks.size; i++)
    {
      ...
      if (!ivec3scmp (chunk->offset, world_pos_to_offset (data.pos)))
        {
          chunk_set_block (chunk, world_pos_to_chunk_pos (data.pos),
                           data.block);
...
world_init (struct World *self)
{
  ...
  self->unloaded_blocks.list = malloc (self->unloaded_blocks.capacity
                                       * sizeof (struct WorldUnloadedBlock));

  self->chunks_size = 16;
  self->chunks = calloc (1, NUM_CHUNKS (self) * sizeof (struct Chunk *));
  self->heightmaps
      = calloc (1, NUM_HEIGHTMAPS (self) * sizeof (struct Heightmap *));

  world_set_center (self, GLMS_IVEC3_ZERO);
}

void
world_destroy (struct World *self)
{
...
void
world_append_unloaded_block (struct World *self, ivec3s pos,
                             enum BlockId block)
{
  ...
  self->unloaded_blocks.list[self->unloaded_blocks.size++]
      = (struct WorldUnloadedBlock){ .pos = pos, .block = block };
}

void
world_remove_unloaded_block (struct World *self, size_t i)
{
  ...
  if (i != self->unloaded_blocks.size)
    {
      memmove (self->unloaded_blocks.list + (i + 0),
               self->unloaded_blocks.list + (i + 1),
               (self->unloaded_blocks.size - i)
                   * sizeof (struct WorldUnloadedBlock));
    }
}
...
static void
load_empty_chunks (struct World *self)
{
...
void
world_set_center (struct World *self, ivec3s center_pos)
{
  ivec3s new_offset = world_pos_to_offset (center_pos);
  ivec3s new_origin = glms_ivec3_sub (
      new_offset, (ivec3s){ { (self->chunks_size / 2), (self->chunks_size / 2),
                              (self->chunks_size / 2) } });
  ...
  // Place old chunks in positions of new chunk array, destroy if they are
  // out of bounds now
  ...
  memcpy (old_heightmaps, self->heightmaps,
          NUM_HEIGHTMAPS (self) * sizeof (struct Heightmap *));
  memset (self->heightmaps, 0,
          NUM_HEIGHTMAPS (self) * sizeof (struct Heightmap *));
      ...
      if (self->heightmaps[i] == NULL)
        {
          ...
          self->heightmaps[i]->data
              = malloc (CHUNK_SIZE.x * CHUNK_SIZE.z * sizeof (i64));
          self->heightmaps[i]->worldgen_data = malloc (
              CHUNK_SIZE.x * CHUNK_SIZE.z * sizeof (struct WorldgenData));
          memsetl (self->heightmaps[i]->data, HEIGHTMAP_UNKNOWN,
                   CHUNK_SIZE.x * CHUNK_SIZE.z * sizeof (i64));
        }
    }
...
void
world_render (struct World *self)
{
  // copy camera settings from view entity
  struct CameraComponent *c_camera = ecs_get (self->entity_view, C_CAMERA);
  memcpy (&state.renderer.perspective_camera, &c_camera->camera,
          sizeof (struct PerspectiveCamera));
  ...
  shader_uniform_texture2D (state.renderer.shaders[SHADER_CHUNK], "tex",
                            state.renderer.block_atlas.atlas.texture, 0);
  shader_uniform_vec4 (state.renderer.shaders[SHADER_CHUNK], "sunlight_color",
                       self->sky.sunlight_color);

  shader_uniform_vec4 (state.renderer.shaders[SHADER_CHUNK], "fog_color",
                       self->sky.fog_color);
  shader_uniform_float (state.renderer.shaders[SHADER_CHUNK], "fog_near",
                        self->sky.fog_near);
  shader_uniform_float (state.renderer.shaders[SHADER_CHUNK], "fog_far",
                        self->sky.fog_far);
...
void
world_update (struct World *self)
{
  // reset throttles
...
void
world_tick (struct World *self)
{
  self->ticks++;

  world_set_center (self, ((struct PositionComponent *)ecs_get (
                               self->entity_view, C_POSITION))
                              ->block);
...
size_t
world_get_aabbs (struct World *self, AABB area, AABB *aabbs, size_t n)
{
