...
f32
octave_compute (struct Octave *p, f32 seed, f32 x, f32 z)
{
  f32 u = 1.0f, v = 0.0f;
  for (int i = 0; i < p->n; i++)
    {
      v += (1.0f / u)
           * noise3 ((x / 1.01f) * u, (z / 1.01f) * u, seed + (p->o * 32));
      u *= 2.0f;
    }
  return v;
}
...
f32
combined_compute (struct Combined *p, f32 seed, f32 x, f32 z)
{
  return p->n->compute (&p->n->params, seed,
                        x + p->m->compute (&p->m->params, seed, x, z), z);
}
...
f32
noise_compute (struct Basic *b, f32 seed, f32 x, f32 z)
{
...
f32
expscale_compute (struct ExpScale *e, f32 seed, f32 x, f32 z)
{
...
struct Noise
expscale (struct Noise *n, f32 exp, f32 scale)
{
