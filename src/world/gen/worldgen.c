...
#define RADIAL2I(c, r, v)                                                     \
  (glms_vec2_norm (glms_vec2_sub (vec2i2f ((c)), vec2i2f ((v))))              \
   / glms_vec2_norm (vec2i2f ((r))))

#define RADIAL3I(c, r, v)                                                     \
  (glms_vec3_norm (glms_vec3_sub (ivec2to3 ((c)), ivec2to3 ((v))))            \
   / glms_vec3_norm (ivec2to3 ((r))))
...
struct BiomeData
    BIOME_DATA[BIOME_LAST + 1] =
        {
            [OCEAN] = {.top_block = SAND, .bottom_block = SAND, .roughness = 1.0f, .scale = 1.0f, .exp = 1.0f},
            [RIVER] = {.top_block = SAND, .bottom_block = SAND, .roughness = 1.0f, .scale = 1.0f, .exp = 1.0f},
            [BEACH] = {.top_block = SAND, .bottom_block = SAND, .roughness = 0.2f, .scale = 0.8f, .exp = 1.3f},
            [DESERT] = {.top_block = SAND,
                        .bottom_block = SAND,
                        .roughness = 0.6f,
                        .scale = 0.6f,
                        .exp = 1.2f,
                        .decorations = {{.f = worldgen_shrub, .chance = 0.005f}}},
            [SAVANNA] = {.top_block = GRASS,
                         .bottom_block = DIRT,
                         .roughness = 1.0f,
                         .scale = 1.0f,
                         .exp = 1.0f,
            ...
            [TUNDRA] = {.top_block = SNOW,
                        .bottom_block = STONE,
                        .roughness = 1.0f,
                        .scale = 1.0f,
                        .exp = 1.0f,
                        .decorations = {{.f = worldgen_pine, .chance = 0.0005f}}},
            [ICE] = {.top_block = SNOW, .bottom_block = STONE, .roughness = 1.0f, .scale = 1.0f, .exp = 1.0f},
            [MOUNTAIN] = {.top_block = SNOW, .bottom_block = STONE, .roughness = 2.0f, .scale = 1.2f, .exp = 1.0f},
};
...
const f32 HEAT_MAP[] = { 0.05f, 0.18f, 0.4f, 0.6f, 0.8f };

const f32 MOISTURE_MAP[] = { 0.2f, 0.3f, 0.5f, 0.6f, 0.7f };
...
static enum Biome
get_biome (f32 h, f32 m, f32 t, f32 n, f32 i)
{
  if (h <= 0.0f || n <= 0.0f)
    {
      return OCEAN;
...
static enum BlockId
_get (struct Chunk *chunk, i32 x, i32 y, i32 z)
{
  ...
  if (chunk_in_bounds (p))
    ...
    {
      return world_get_block (chunk->world,
                              glms_ivec3_add (chunk->position, p));
    }
}

static void
_set (struct Chunk *chunk, i32 x, i32 y, i32 z, u32 d)
{
  ivec3s p = (ivec3s){ { x, y, z } };
  if (chunk_in_bounds (p))
    {
  ...
  if (!heightmap->flags.generated)
    {
      ...
      struct Noise bs[] = { basic (1), basic (2), basic (3), basic (4) };

      struct Noise os[] = {
        octave (5, 0), octave (5, 1), octave (5, 2),
        octave (5, 3), octave (5, 4), octave (5, 5),
      };

      struct Noise cs[]
          = { combined (&bs[0], &bs[1]), combined (&bs[2], &bs[3]),
              combined (&os[3], &os[4]), combined (&os[1], &os[2]),
              combined (&os[1], &os[3]) };
          ...
          for (i64 z = 0; z < CHUNK_SIZE.z; z++)
            {
              ...
              f32 h = n_h.compute (&n_h.params, chunk->world->seed, wx, wz),
                  m = n_m.compute (&n_m.params, chunk->world->seed, wx, wz)
                          * 0.5f
                      + 0.5f,
                  t = n_t.compute (&n_t.params, chunk->world->seed, wx, wz)
                          * 0.5f
                      + 0.5f,
              ...
              heightmap->worldgen_data[x * CHUNK_SIZE.x + z]
                  = (struct WorldgenData){
                      .h_b = ((h * 32.0f) + (n * 256.0f)) * biome.scale
...
#define WG_GET_H(_x, _z)                                                      \
  heightmap                                                                   \
      ->worldgen_data[clampi64 ((_x), 0, CHUNK_SIZE.x - 1) * CHUNK_SIZE.x     \
                  ...
                  for (size_t i = 0; i < MAX_DECORATIONS; i++)
                    {
                      ...
                      if (RANDCHANCE (biome_data.decorations[i].chance))
                        {
                          biome_data.decorations[i].f (chunk, _get, _set, x, y,
                                                       z);
                        }
                    }
                }
