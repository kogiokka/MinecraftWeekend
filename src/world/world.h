...
#define HEIGHTMAP_INDEX(p)                                                    \
  ({                                                                          \
    ivec2s q = (p);                                                           \
    (q.x * CHUNK_SIZE.x) + q.y;                                               \
  })
...
struct World
{
  ...
  // the world is loaded
  // both must have a position component; entity_view must have a camera
  // component
  ...
  // Array of heightmaps for each X-Z chunk column, sizeis always chunks_size ^
  // 2
...
void world_append_unloaded_block (struct World *self, ivec3s pos,
                                  enum BlockId block);
...
static inline ivec3s
world_pos_to_offset (ivec3s pos)
{
...
// float pos -> block pos
static inline ivec3s
world_pos_to_block (vec3s pos)
{
  return (ivec3s){ { (i32)floorf (pos.x), (i32)floorf (pos.y),
                     (i32)floorf (pos.z) } };
}

// world position -> chunk position
static inline ivec3s
world_pos_to_chunk_pos (ivec3s pos)
{
  // ((pos % size) + size) % size
  return glms_ivec3_mod (
      glms_ivec3_add (glms_ivec3_mod (pos, CHUNK_SIZE), CHUNK_SIZE),
      CHUNK_SIZE);
}

static inline bool
world_chunk_in_bounds (struct World *self, ivec3s offset)
{
  ivec3s p = glms_ivec3_sub (offset, self->chunks_origin);
  return p.x >= 0 && p.y >= 0 && p.z >= 0 && p.x < (i32)self->chunks_size
         && p.y < (i32)self->chunks_size && p.z < (i32)self->chunks_size;
}

// chunk offset -> world array index
static inline size_t
world_chunk_index (struct World *self, ivec3s offset)
{
  ivec3s p = glms_ivec3_sub (offset, self->chunks_origin);
  return (p.x * self->chunks_size * self->chunks_size)
         + (p.z * self->chunks_size) + p.y;
}

// world array index -> chunk offset
static inline ivec3s
world_chunk_offset (struct World *self, size_t i)
{
...
world_get_chunk (struct World *self, ivec3s offset)
{
...
static inline bool
world_contains_chunk (struct World *self, ivec3s offset)
{
...
static inline bool
world_in_bounds (struct World *self, ivec3s pos)
{
...
static inline bool
world_contains (struct World *self, ivec3s pos)
{
...
#define WORLD_DECL_DATA(T, _name)                                             \
  ...
  {                                                                           \
    ...
    if (world_contains_chunk (self, offset))                                  \
      {                                                                       \
        return chunk_get_##_name (world_get_chunk (self, offset),             \
                                  world_pos_to_chunk_pos (pos));              \
      }                                                                       \
    return (T)0;                                                              \
  }                                                                           \
  static inline void world_set_##_name (struct World *self, ivec3s pos,       \
                                        T value)                              \
  {                                                                           \
    ivec3s offset = world_pos_to_offset (pos);                                \
    if (world_contains_chunk (self, offset))                                  \
      {                                                                       \
        chunk_set_##_name (world_get_chunk (self, offset),                    \
                           world_pos_to_chunk_pos (pos), value);              \
      }                                                                       \
  }
