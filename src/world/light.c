...
static void
add_propagate (struct World *world, struct LightQueue *queue, u32 mask,
               u32 offset, enum PropagationType type)
{
  while (queue->size != 0)
    {
      ...
      for (enum Direction d = DOWN; (i32)d >= 0; d--)
        {
          ...
          // TODO: it may be incorrect to check n_val != 0 to subvert the block
          // behavior call
          if ((n_val != 0 || n_block.transparent)
              && ((sunlight_down && n_val < val) || (n_val + 1 < val)))
            {
              ...
              world_set_light (
                  world, n_pos,
                  (n_light & ~mask)
                      | ((((light & mask) >> offset) + delta) << offset));
...
static void
remove_propagate (struct World *world, struct LightQueue *queue,
                  struct LightQueue *prop_queue, u32 mask, u32 offset,
                  enum PropagationType type)
{
  while (queue->size != 0)
    {
      struct LightNode node = DEQUEUE (queue);
      ...
      for (enum Direction d = 0; d < 6; d++)
        {
          ...
          if ((n_light & mask) != 0
              && (n_value < value || (type == SUNLIGHT && d == DOWN)))
            {
              ...
              ENQUEUE (queue,
                       ((struct LightNode){ .pos = n_pos, .value = n_value }));
            }
...
static void
add_channel (struct World *world, ivec3s pos, u8 value, u32 mask, u32 offset,
             enum PropagationType type)
{
  struct LightQueue queue = { .size = 0 };
  world_set_light (world, pos,
                   (world_get_light (world, pos) & ~mask)
                       | (((u32)value) << offset));
...
static void
remove_channel (struct World *world, ivec3s pos, u32 mask, u32 offset,
                enum PropagationType type)
{
  struct LightQueue queue = { .size = 0 }, prop_queue = { .size = 0 };

  u32 light = world_get_light (world, pos);
  world_set_light (world, pos, light & ~mask);

  ENQUEUE (&queue, ((struct LightNode){ .pos = pos,
                                        .value = (light & mask) >> offset }));
  remove_propagate (world, &queue, &prop_queue, mask, offset, type);
  add_propagate (world, &prop_queue, mask, offset, type);
}

void
torchlight_add (struct World *world, ivec3s pos, Torchlight light)
{
  ...
  for (size_t i = 0; i < 4; i++)
    {
      ...
      add_channel (world, pos, (light & mask) >> offset, mask, offset,
                   DEFAULT_LIGHT);
    }
}

void
torchlight_remove (struct World *world, ivec3s pos)
{
  for (size_t i = 0; i < 4; i++)
    {
...
void
light_update (struct World *world, ivec3s pos)
{
  struct LightQueue queue = { .size = 0 };
  ...
  for (size_t i = 0; i < 5; i++)
    {
      ...
      add_propagate (world, &queue, mask, offset,
                     sunlight ? SUNLIGHT : DEFAULT_LIGHT);
    }
}

void
light_remove (struct World *world, ivec3s pos)
{
...
void
light_apply (struct Chunk *chunk)
{
          ...
          for (i64 y = CHUNK_SIZE.y - 1; y >= 0; y--)
            {
                  ...
                  for (enum Direction d = NORTH; d <= WEST; d++)
                    {
                      ...
                      if (pos_w.y
                          < (chunk_in_bounds (pos_cn)
                                 ? HEIGHTMAP_GET (
                                     heightmap,
                                     ((ivec2s){ { pos_cn.x, pos_cn.z } }))
                                 : world_heightmap_get (
                                     chunk->world,
                                     ((ivec2s){ { pos_wn.x, pos_wn.z } }))))
                        {
                          ENQUEUE (&sunlight_queue,
                                   ((struct LightNode){ .pos = pos_w }));
                        }
                    }
                }
              ...
              if (block.can_emit_light)
                {
                  Torchlight value
                      = block.get_torchlight (chunk->world, pos_w);
                  chunk_set_torchlight (chunk, pos_c, value);
                  ENQUEUE (
                      &torchlight_queue,
                      ((struct LightNode){ .pos = pos_w, .value = value }));
                }
            }
        }
    }
      ...
      for (i64 z = 0; z < CHUNK_SIZE.z; z++)
        {
          ...
          if (light != 0)
            {
              ENQUEUE (&torchlight_queue,
                       ((struct LightNode){ .pos = pos, .value = light }));
            }

          pos = glms_ivec3_add (chunk->position,
                                (ivec3s){ { x, CHUNK_SIZE.y, z } });
          light = world_get_light (chunk->world, pos);
