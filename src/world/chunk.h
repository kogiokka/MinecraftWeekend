...
#define chunk_foreach(_pname)                                                 \
  ...
  for (i32 x = 0; x < CHUNK_SIZE.x; x++)                                      \
    for (i32 z = 0; z < CHUNK_SIZE.z; z++)                                    \
      for (i32 y = 0;                                                         \
           y < CHUNK_SIZE.y && ((_pname.x = x) != INT32_MAX)                  \
           && ((_pname.y = y) != INT32_MAX) && ((_pname.z = z) != INT32_MAX); \
           y++)

#define chunk_pos_to_index(p)                                                 \
  (p.x * CHUNK_SIZE.x * CHUNK_SIZE.z + p.z * CHUNK_SIZE.z + p.y)

// returns true if pos is within chunk boundaries
static inline bool
chunk_in_bounds (ivec3s pos)
{
  return pos.x >= 0 && pos.y >= 0 && pos.z >= 0 && pos.x < CHUNK_SIZE.x
         && pos.y < CHUNK_SIZE.y && pos.z < CHUNK_SIZE.z;
}

// returns true if pos is on chunk boundaries (borders another chunk)
static inline bool
chunk_on_bounds (ivec3s pos)
{
  return pos.x == 0 || pos.y == 0 || pos.z == 0 || pos.x == (CHUNK_SIZE.x - 1)
         || pos.y == (CHUNK_SIZE.y - 1) || pos.z == (CHUNK_SIZE.z - 1);
}
...
#define CHUNK_DECL_DATA(T, _name, _mask, _offset)                             \
  static inline T chunk_get_##_name (struct Chunk *self, ivec3s pos)          \
  {                                                                           \
    return (T)((self->data[chunk_pos_to_index (pos)] & _mask) >> _offset);    \
  }                                                                           \
  static inline void chunk_set_##_name (struct Chunk *self, ivec3s pos,       \
                                        T value)                              \
  {                                                                           \
    size_t index = chunk_pos_to_index (pos);                                  \
    u64 data = self->data[index];                                             \
    u64 new_data = (data & ~(_mask)) | ((((u64)value) << _offset) & _mask);   \
    self->data[index] = new_data;                                             \
    chunk_on_modify (self, pos, data, new_data);                              \
  }                                                                           \
  static inline T chunk_data_to_##_name (u64 value)                           \
  {                                                                           \
